...
class OtterAIRunner:
    ...
    def __init__(self):
        ...
        self.methods = {
            "1": {"name": "get_user", "desc": "Get current user information", "params": []},
            "2": {"name": "get_speakers", "desc": "Get all speakers", "params": []},
            "3": {"name": "get_speeches", "desc": "Get speeches list", "params": ["folder", "page_size", "source"]},
            "4": {"name": "get_speech", "desc": "Get specific speech by OTID", "params": ["otid"]},
            "5": {"name": "query_speech", "desc": "Query speech content", "params": ["query", "otid", "size"]},
            "6": {"name": "get_notification_settings", "desc": "Get notification settings", "params": []},
            "7": {"name": "list_groups", "desc": "List all groups", "params": []},
            "8": {"name": "get_folders", "desc": "Get all folders", "params": []},
            "9": {"name": "set_speech_title", "desc": "Set speech title", "params": ["otid", "title"]},
            "10": {"name": "create_speaker", "desc": "Create new speaker", "params": ["speaker_name"]},
            "11": {"name": "move_to_trash_bin", "desc": "Move speech to trash", "params": ["otid"]},
            "12": {"name": "download_speech", "desc": "Download speech", "params": ["otid", "name", "fileformat"]},
            "13": {"name": "get_contacts_structured", "desc": "Get contacts (structured)", "params": []},
            "14": {"name": "get_folders_structured", "desc": "Get folders (structured)", "params": []},
            "15": {"name": "get_speech_mention_candidates_structured", "desc": "Get mention candidates (structured)", "params": ["otid"]},
        }
    ...
    def display_menu(self):
        ...
        for key, method in self.methods.items():
            params_str = f" (requires: {', '.join(method['params'])})" if method['params'] else ""
    ...
    def get_method_parameters(self, method_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for param in method_info["params"]:
            if param == "otid":
                ...
                if default_otid:
                    value = self.get_user_input(f"Enter {param} (default: {default_otid}): ", required=False)
                    params[param] = value or default_otid
                else:
                    params[param] = self.get_user_input(f"Enter {param}: ")
            elif param == "folder":
                value = self.get_user_input(f"Enter {param} (default: 0): ", required=False)
                params[param] = int(value) if value else 0
            elif param == "page_size":
                value = self.get_user_input(f"Enter {param} (default: 45): ", required=False)
                params[param] = int(value) if value else 45
            elif param == "size":
                value = self.get_user_input(f"Enter {param} (default: 500): ", required=False)
                params[param] = int(value) if value else 500
            elif param == "source":
                value = self.get_user_input(f"Enter {param} (default: owned): ", required=False)
                params[param] = value or "owned"
            elif param == "fileformat":
                value = self.get_user_input(f"Enter {param} (default: txt,pdf,mp3,docx,srt): ", required=False)
    ...
    def save_to_file(self, content: str, method_name: str) -> Optional[str]:
        ...
        save_choice = self.get_user_input(f"💾 Save to file? (y/N, default filename: {filename}): ", required=False)
        if save_choice and save_choice.lower().startswith('y'):
            custom_filename = self.get_user_input(f"Enter filename (default: {filename}): ", required=False)
