...
class InteractiveRunner:
    def __init__(self):
        ...
        self.available_apis = {
            "abstract_summary": {
                ...
                "requires_auth": True,
                "requires_otid": True,
                "method": lambda otid: {"status": 200, "data": {"message": "Abstract summary placeholder - not implemented in current API"}}
            },
            "applied_speech_template": {
                ...
                "requires_auth": True,
                "requires_otid": True,
                "method": lambda otid: {"status": 200, "data": {"message": "Applied speech template placeholder - not implemented in current API"}}
            },
            "get_user": {
                "name": "get_user",
            ...
            "set_speech_title": {
                ...
                "requires_auth": True,
                "requires_otid": True,
                "method": lambda otid: self.client.set_speech_title(otid, "Updated Title")
            },
            "move_to_trash_bin": {
                ...
                for num in numbers:
                    if 1 <= num <= len(api_options):
                        ...
                        if api_key not in selected_apis:
                            ...
                            console.print(f"[green]✅ Added: {self.available_apis[api_key]['name']}[/green]")
                        else:
                            console.print(f"[yellow]⚠️ Already selected: {self.available_apis[api_key]['name']}[/yellow]")
                    else:
    ...
    def get_test_otid(self) -> Optional[str]:
        ...
        otid = Prompt.ask("\n[bold]Enter a speech OTID for testing (or press Enter to skip APIs requiring OTID)[/bold]", default="")
    ...
    def display_response(self, response: Dict, api_name: str):
        ...
        if response["status"] == "success":
            ...
            console.print(Panel(syntax, title=f"✅ {api_name} Response", border_style="green"))
        elif response["status"] == "error":
            console.print(Panel(f"❌ Error: {response['error']}", border_style="red"))
        elif response["status"] == "skipped":
            console.print(Panel(f"⏭️ Skipped: {response['reason']}", border_style="yellow"))
    ...
    def run(self):
        ...
        if needs_auth and not self.login():
            console.print("[red]❌ Cannot proceed without login for authenticated APIs[/red]")
            return
        ...
        # Get OTID if needed
        otid = None
        needs_otid = any(self.available_apis[api_key]["requires_otid"] for api_key in selected_apis)
        if needs_otid:
        ...
        for i, api_key in enumerate(selected_apis):
            if i > 0:
                console.print("[blue]⏱️ Waiting 2 seconds to avoid rate limits...[/blue]")
                time.sleep(2)
